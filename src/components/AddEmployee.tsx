...
export default function AddEmployee() {
  ...
  function validateStep(step: string) {
    switch (step) {
      case "details":
        ...
        if (!personalDetails.mobile.match(/^\d{10}$/))
          return "Mobile number must be 10 digits";
        if (!personalDetails.email.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/))
      ...
      case "documents":
        if (!documentFiles.aadhaarNo.match(/^\d{12}$/))
          return "Aadhaar number must be 12 digits";
        if (!documentFiles.aadhaar) return "Aadhaar card file is required";
        if (!documentFiles.panNo.match(/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/))
          return "PAN number format is invalid";
        if (!documentFiles.pan) return "PAN card file is required";
        return null;

      case "education":
        if (!educationDetails.qualification.trim())
          return "Qualification is required";
        if (!educationDetails.institute.trim()) return "Institute is required";
        if (!educationDetails.yearOfPassing.match(/^\d{4}$/))
          return "Year of passing must be 4 digits";
        if (educationDetails.percentage < 0 || educationDetails.percentage > 100)
      ...
      case "job":
        ...
        if (!jobDetails.amount || jobDetails.amount <= 0)
          return "Amount must be greater than 0";
        if (!jobDetails.paymentFrequency)
  ...
  async function handleSubmit() {
    ...
    if (educationDetails.marksheet)
      formData.append("Marksheet", educationDetails.marksheet);
...
function PersonalDetails({
  ...
  const employeePicUploader = useRef<HTMLInputElement | null>(null);
  const [employeePic, setEmployeePic] = useState<string | null>(
    personalDetails.pic ? URL.createObjectURL(personalDetails.pic) : null
  );
...
function Documents({
  ...
  const aadhaarCardRef = useRef<HTMLInputElement | null>(null);
  const panCardRef = useRef<HTMLInputElement | null>(null);
  const voterCardRef = useRef<HTMLInputElement | null>(null);
